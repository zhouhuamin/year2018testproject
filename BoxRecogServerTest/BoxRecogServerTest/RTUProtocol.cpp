...
bool RTUProtocol::ReadUploadData(const std::vector<BYTE> &dataVect, std::pair<BYTE, structUploadData> &pairData, const std::string &strReverse)
{
	BYTE ADDR	= 0x01;
	BYTE CMD	= 0x53;
	...
	else
	{
        ...
        if (strReverse == "1")
        {
            upLoadData.Di_Last  = ~upLoadData.Di_Last;
            upLoadData.Di_Now   = ~upLoadData.Di_Now;
        }
