...
int CMSG_Center::handleDeviceRegister(NET_PACKET_MSG *pPacket)
{
    ...
    int sendLen = 0;
    memcpy(chMsgAck, SYS_NET_MSGHEAD, 8); //��ͷ���?
    sendLen += 8;
...
INDEX_STEP:
    ...
    if (CSimpleConfig::m_StaSequenceControlVec.size() > 0)
    {
        for (int i = 0; i < CSimpleConfig::m_StaSequenceControlVec.size(); i++)
        {
            ...
            {
                if (strcmp(pSequence->recv_event[j].event_id, pEventData->event_id) == 0) //��ݽ��յ����¼�ID�����Ҷ�Ӧ�ķ����¼�ID
                {
                    nEventIndex = j;
                    break;
                }
...
if (CSimpleConfig::m_StaSequenceControlVec.size() > 0)
{
    ...
    pPacket_->msg_head.msg_type = SYS_MSG_SYSTEM_MSG_SEQUENCE_DISPATCH; //�����յ���ݣ�put���¼����м�����
   ...
   // syslog(LOG_DEBUG, "Enter handle_event\n");
    if (CSimpleConfig::m_StaSequenceControlVec.size() > 0)
    {
        ...
        for (int i = 0; i < CSimpleConfig::m_StaSequenceControlVec.size(); i++)
        {
            ...
            for (int j = 0; j < pSequence->recv_event_num; j++)
            {
                if (strcmp(pSequence->recv_event[j].event_id, pSequenceData->event_id) == 0) //��ݽ��յ����¼�ID�����Ҷ�Ӧ�ķ����¼�ID
                {
                    nEventIndex = j;
                    break;
                }
            ...
            if (pSequence->event_assoc == 1) //�¼�֮������Ĺ�ϵ,����ȫ������
            {
                for (int m = 0; m < pSequence->recv_event_num; m++)
                {
...
int CMSG_Center::handle_device_ctrlevent(char* szEventID, char* szCtrlData)
{
    ...
    if (CSimpleConfig::m_StaSequenceControlVec.size() > 0)
    {
        ...
        for (int i = 0; i < CSimpleConfig::m_StaSequenceControlVec.size(); i++)
        {
            ...
            for (int j = 0; j < pSequence->recv_event_num; j++)
            {
                if (strcmp(pSequence->recv_event[j].event_id, szEventID) == 0) //��ݽ��յ����¼�ID�����Ҷ�Ӧ�ķ����¼�ID
                {
                    nEventIndex = j;
                    break;
                }
...

                {
                    ...
                    if (CSimpleConfig::m_StaSequenceControlVec.size() > 0)
                    {
                        for (int i = 0; i < CSimpleConfig::m_StaSequenceControlVec.size(); i++)
                        {
                            ...
                            for (int j = 0; j < pSequence->recv_event_num; j++)
                            {
                                if (strcmp(pSequence->recv_event[j].event_id, sequenceData->event_id) == 0) //��ݽ��յ����¼�ID�����Ҷ�Ӧ�ķ����¼�ID
                                {
...
{
    if (strcmp(device_tag, "CTRL") == 0) //CTRLĬ��Ϊ���ؿ��ƴ���
    {
        return 0;
    }
