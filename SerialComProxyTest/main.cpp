...
//int rsmain()
{
    ...
    openlog("serialcomproxy", LOG_PID, LOG_LOCAL7);
...
//    }
    ...
    int nCUSTOMS_PLATFORM_LISTEN_PORT = atoi(CSimpleConfig::CUSTOMS_PLATFORM_LISTEN_PORT.c_str());
    CPlat_Cmd_Acceptor::InitHandler();
    CPlat_Cmd_Acceptor plat_acceptor;
    if (plat_acceptor.open(ACE_INET_Addr(nCUSTOMS_PLATFORM_LISTEN_PORT), reactor, 1) == -1)
    {
        //ACE_ERROR_RETURN((LM_ERROR, "%p\n", "open listen port fail..."), -1);
        syslog(LOG_DEBUG, "open listen port fail...");
        return -1;
    }
    else
    {
        //ACE_DEBUG((LM_DEBUG, "(%P|%t) open listen port %d ok...\n", nCUSTOMS_PLATFORM_LISTEN_PORT));
        syslog(LOG_DEBUG, "open listen port %d ok...\n", nCUSTOMS_PLATFORM_LISTEN_PORT);
    }
    ...
    syslog(LOG_DEBUG, "starting up ccs server daemon\n");
